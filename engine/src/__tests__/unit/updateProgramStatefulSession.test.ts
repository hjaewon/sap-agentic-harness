/**
 * Regression test for the UpdateProgram lock-session bug (vsp issue #88 class).
 *
 * SAP-independent: drives the real high-level handleUpdateProgram through the
 ...
 *
 * The bug: getProgram().lock() leaves the connection stateless, so the
...
describe('UpdateProgram stateful lock session (regression: invalid lock handle / HTTP 423)', () => {
  const SOURCE = "REPORT zsah_test.\nWRITE / 'ok'.";
  ...
  it('keeps the connection stateful across lock -> pre-write check -> source PUT', async () => {
    ...
    const result = await handleUpdateProgram(context, {
      program_name: 'ZSAH_TEST',
      source_code: SOURCE,
    });
