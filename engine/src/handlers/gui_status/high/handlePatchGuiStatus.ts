  ...
  inputSchema: {
    ...
    properties: {
      ...
      skip_validation: {
        type: 'boolean',
        description:
          'Skip client-side validation of the merged result. Default: false.',
      },
    },
...
interface PatchGuiStatusArgs {
  program_name: string;
  changes: string | CuaData | unknown;
 ...
 */
function extractCurrentCua(readResponse: any): CuaData {
  const text = readResponse?.content?.find((c: any) => c.type === 'text')?.text;
  if (!text) throw new Error('ReadGuiStatus returned empty body');
  ...
  return def as CuaData;
}

export async function handlePatchGuiStatus(
  context: HandlerContext,
  params: any,
) {
  const { connection, logger } = context;
  const args: PatchGuiStatusArgs = params;
  ...
  if (
    ...
    return return_error(
      new Error('Missing required parameters: program_name and changes'),
    );
  }
  ...
  try {
    ...
    return return_error(
      new Error(`Invalid changes: ${e?.message || String(e)}`),
    );
  }
  ...
  try {
    ...
    const fetchResp = await callDispatch(connection, 'CUA_FETCH', {
      program: programName,
    });
    let current: CuaData;
    try {
      ...
      if (result && typeof result === 'object') {
        current = result as CuaData;
      } else if (typeof result === 'string') {
        current = normalizeCuaInput(result);
      } else {
        current = {};
      }
    } catch (e: any) {
      throw new Error(
        `Failed to parse current CUA for ${programName}: ${e?.message || e}`,
      );
    }
    ...
    if (args.skip_validation !== true) {
      ...
      if (hard.length > 0) {
        ...
        throw new Error(
          `PatchGuiStatus rejected — merged cua has ${hard.length} validation problem(s). Fix changes (or pass skip_validation=true):\n${summary}`,
        );
      }
    ...
    const countRows = (arr: unknown): number =>
    ...
    return return_error(
      new Error(`Failed to patch GUI status: ${errorMessage}`),
    );
  }
}
