...
function detectLegacy(): boolean {
  if (process.env.SAP_SYSTEM_TYPE?.toLowerCase() === 'legacy') return true;
  if (process.env.SAP_VERSION?.toUpperCase() === 'ECC') return true;
  const release = parseInt(process.env.ABAP_RELEASE || '', 10);
  if (Number.isFinite(release) && release < 750) return true;
  return false;
}
