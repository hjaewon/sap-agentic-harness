 ...
 */
export function compressFunctionModule(source: string): string {
  const lines = source.split(/\r?\n/);

  const funcIdx = lines.findIndex((line) =>
    /^\s*FUNCTION\s+\S+\s*\.\s*$/i.test(line),
  );
  if (funcIdx === -1) {
    return source;
  }
  ...
  const endIdx = lines.findIndex(
    (line, i) => i > funcIdx && /^\s*ENDFUNCTION\s*\.\s*$/i.test(line),
  );
  if (endIdx === -1) {
    return source;
  }

  const kept: string[] = [lines[funcIdx]];
