...
export function scanAbapDependencies(source: string): AbapDependencies {
  ...
  function pushCandidate(rawName: string, kind: Kind, priority: number): void {
  ...
  function pushTypeCandidate(rawName: string, priority: number): void {
    const name = rawName.toUpperCase();
