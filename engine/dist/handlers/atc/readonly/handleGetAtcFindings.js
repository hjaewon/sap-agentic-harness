...
exports.handleGetAtcFindings = handleGetAtcFindings;
/**
...
const atcWorklistParser_1 = require("../../../lib/atcWorklistParser");
const resolveAdtUri_1 = require("../../../lib/resolveAdtUri");
...
exports.TOOL_DEFINITION = {
    ...
    inputSchema: {
        ...
        properties: {
            object_uri: {
                type: 'string',
                description: 'Explicit ADT URI of the target (e.g. /sap/bc/adt/oo/classes/zcl_x). If given, object_name/object_type are ignored.',
            },
            object_name: {
                type: 'string',
                description: 'Object name (used with object_type when object_uri is absent).',
            },
            object_type: {
                type: 'string',
                description: 'SAP object type code for URI resolution: CLAS, INTF, PROG, FUGR, TABL, STRU, VIEW, DTEL, DOMA, DDLS, BDEF, SRVD, SRVB, DEVC (package).',
            },
            check_variant: {
                type: 'string',
                description: 'ATC check variant name. If omitted, the system default variant is resolved from /atc/customizing.',
            },
            max_results: {
                type: 'number',
...
async function handleGetAtcFindings(context, args) {
    ...
    try {
        ...
        if (args.object_uri && args.object_uri.trim().length > 0) {
            ...
            mainUrl = (0, resolveAdtUri_1.resolveAdtUri)({
                name: args.object_name,
                type: args.object_type,
            });
        }
        else {
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, 'Provide object_uri, or object_name + object_type.');
        }
        const maxResults = args.max_results ?? 100;
        // Resolve check variant (explicit input wins; else system default).
        let variant = args.check_variant?.trim();
        if (!variant) {
            const cust = await (0, utils_1.makeAdtRequestWithTimeout)(connection, `${ATC_BASE}/customizing`, 'GET', 'default', undefined, undefined, {
            ...
            const hit = arr.find((p) => String(p?.['@_name'] ?? '').toLowerCase() === 'systemcheckvariant');
            variant = hit?.['@_value'] ? String(hit['@_value']) : undefined;
            if (!variant) {
                throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, 'No check_variant given and could not resolve the system default from /atc/customizing — pass check_variant explicitly.');
            }
        }
        logger?.info(`ATC: variant=${variant} target=${mainUrl} max=${maxResults}`);
        // 2. Create a worklist for the variant -> worklistId (text/plain).
        const worklistResp = await (0, utils_1.makeAdtRequestWithTimeout)(connection, `${ATC_BASE}/worklists?checkVariant=${encodeURIComponent(variant)}`, 'POST', 'csrf', undefined, undefined, { Accept: 'text/plain' });
  ...
  <objectSets xmlns:adtcore="http://www.sap.com/adt/core">
    <objectSet kind="inclusive">
      <adtcore:objectReferences>
        ...
        const runResp = await (0, utils_1.makeAdtRequestWithTimeout)(connection, `${ATC_BASE}/runs?worklistId=${encodeURIComponent(worklistId)}`, 'POST', 'long', runBody, undefined, { Accept: 'application/xml', 'Content-Type': 'application/xml' });
        ...
        const findingsResp = await (0, utils_1.makeAdtRequestWithTimeout)(connection, `${ATC_BASE}/worklists/${encodeURIComponent(runResultId)}`, 'GET', 'long', undefined, { includeExemptedFindings: false }, { Accept: 'application/atc.worklist.v1+xml' });
        const parsed = (0, atcWorklistParser_1.parseAtcWorklist)(String(findingsResp.data));
        logger?.info(`ATC done: ${parsed.total} findings (E:${parsed.errors} W:${parsed.warnings} I:${parsed.infos})`);
        return {
            isError: false,
            content: [
                {
        ...
        logger?.error('GetAtcFindings failed', error);
        return {
            isError: true,
            content: [
                {
...
//# sourceMappingURL=handleGetAtcFindings.js.map