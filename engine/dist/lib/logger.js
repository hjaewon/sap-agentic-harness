...
exports.connectionManagerLogger = exports.handlerLogger = exports.logger = void 0;
/**
        ...
        if (key === 'config' && value && typeof value === 'object') {
            ...
            for (const [configKey, configValue] of Object.entries(value)) {
                if (configKey === 'headers' &&
                    ...
                    const sanitizedHeaders = {};
                    for (const [headerKey, headerValue] of Object.entries(configValue)) {
                        if (headerKey.toLowerCase() === 'authorization' ||
                            headerKey.toLowerCase() === 'cookie' ||
                            headerKey.toLowerCase() === 'x-csrf-token') {
                            sanitizedHeaders[headerKey] = '[REDACTED]';
...
function createLogFn(level) {
    return (message, data) => {
        ...
        if (level === 'ERROR' &&
            ...
            const allData = typeof responseData === 'string'
                ? responseData
...
exports.logger = {
    ...
    tlsConfig: (rejectUnauthorized) => {
        ...
        const debugEnabled = process.env.DEBUG_CONNECTORS === 'true';
        if (debugEnabled) {
            const message = `TLS certificate validation is ${rejectUnauthorized ? 'enabled' : 'disabled'}`;
            process.stderr.write(`${JSON.stringify({
                level: 'INFO',
                timestamp: new Date().toISOString(),
                type: 'TLS_CONFIG',
 ...
 */
function createHandlerLogFn(level) {
    return (handlerName, step, message, data) => {
        // Never log in stdio mode
        const isStdio = process.env.MCP_TRANSPORT === 'stdio' ||
            process.argv.includes('--transport=stdio') ||
            process.argv.includes('--stdio');
 ...
 */
function createConnectionManagerLogFn(level) {
    return (message, data) => {
        // Never log in stdio mode
        const isStdio = process.env.MCP_TRANSPORT === 'stdio' ||
            process.argv.includes('--transport=stdio') ||
...
exports.connectionManagerLogger = {
    debug: (message, data) => {
