...
## 5. 결정 (사용자 — 통합 선결, 종속 순) — **2026-07-19 답변 반영**
...
1. **방향**: 무인 상시 개방(로컬 D-023) vs attended 중심·무인 봉인(원격 재기준).
   ...
   **→ ✅ 절충안 확정(사용자: "대화형 중심이되 무인도 사용 가능")** — 대화형
   중심 틀(원격 재기준)을 정본으로 채택하고, 무인은 틀 안의 관문(U-gate) 통과로
   사용 가능. 로컬 Phase 3 실증 + D-023 승인이 관문 재료. **로컬 D-023은 "무조건
   상시 개방"이 아니라 이 틀 안의 조건부 개방으로 재기술**(통합 세션에서 새
   D-항목으로 supersede).
2. **리뷰 게이트 정본**: 캡슐 해시(로컬) vs run-scoped(원격 S2 격상판). 폐기
   측 phase 산출물 처분 포함.
   **→ ✅ 역할 분담안 확정(2026-07-19 사용자)** — 원격 run-scoped 틀 = 공통
   골격(베이스 정합), 무인 경로의 SAP write에만 로컬 캡슐 밀봉을 관문 부품으로
   편입(무인 시 배포물 봉인 보증 — ⑴ 절충안과 정합). 양쪽 phase 산출물은 이력
   보존(폐기 없음), 배선 설계는 통합 세션에서 소규모 설계 후 반영.
3. **결정 로그 구조**: `docs/reference/` 이전 수용 여부 + ADR.md 존폐(원격 D-026은
   ...
   **→ ✅ 권고안 확정(사용자 "추천대로")** — 원격 구조 채택(정본 =
   `docs/reference/DECISIONS.md`), 로컬 D-020~023 4건을 재번호로 재기술·supersede
   표기(원격이 D-030을 vsp 편입용으로 예약했으므로 **D-031~034 권장**), 로컬
   ADR.md 내용은 보존하되 top-level에서 `docs/reference/`로 이동(이중 체계
   해소 방식은 통합 세션에서 원격 D-026 논거와 정합 처리).
4. **vsp 편입**(D-018 재론): 원격 브리핑 완비(두 기둥 중 "업스트림 결별 비용"
   ...
   **→ ✅ 편입 확정(사용자가 "다 녹이려던 것" 의사 재확인)** — 타이밍은
   오케스트레이터 권고 = **분기 통합 완료 직후**(subtree 편입은 레포 구조
   작업이라 통합 후가 깨끗). 통합 세션에서 D-030 append.
...
**→ ✅ 채택(2026-07-19 사용자: "주 머신 줄기를 바탕으로 + 이 컴퓨터의 우월 자산을
얹기 나도 이게 맞는 것 같아")** — 병합 베이스 = 원격 main, 이식 자산 = §4 표의
로컬 우월분(엔진 4.13.15·vsp lock 94·FI 팩·지식 이식·[⑵ 결과에 따라] 캡슐 게이트).
