...
export const TOOL_DEFINITION = {
  ...
  description:
    'Release an ABAP transport request or task via the ADT CTS release action. ' +
  ...
  inputSchema: {
    ...
    properties: {
      transport_number: {
        type: 'string',
        description:
          'Transport request or task number to release (e.g., E19K905635, DEVK905123).',
      },
    },
...
interface ReleaseTransportArgs {
...
export function parseReleaseJobResponse(xml: string): ReleaseJobResult {
  ...
  try {
    ...
    const reportsContainer =
      root?.['tm:releasereports'] ||
      root?.['tm:releasereport'] ||
      root?.['tm:releasejob'];
    const reports =
      (reportsContainer && reportsContainer['tm:releasereport']) ||
      reportsContainer;
...
function pickText(value: unknown): string | null {
  ...
  if (typeof value === 'object' && '#text' in (value as any)) {
    return pickText((value as any)['#text']);
  }
  return null;
}
...
function collectMessages(node: unknown, out: string[]): void {
  ...
  for (const item of items) {
    ...
    if (item && typeof item === 'object') {
      const msg =
        pickText((item as any)['tm:message']) ??
        pickText((item as any)['chkrun:shortText']) ??
        pickText((item as any)['@_tm:message']) ??
        pickText((item as any)['#text']);
      if (msg) out.push(msg);
    }
  }
}
  ...
  try {
    ...
    try {
      ...
      if (status === 404 || status === 405) {
        logger?.info(
          `ReleaseTransport: ADT release action not available (HTTP ${status})`,
        );
        return {
          isError: false,
          content: [
