...
- **다음 착수 = 분기 통합 방향 결정** (2026-07-19 분기 발견·조사 완료). 정본 =
  docs/reference/audits/2026-07-19-branch-divergence-assessment.md — 사용자
  결정 4건: ⑴ 방향(무인 개방[로컬 D-023] vs attended 봉인[원격 재기준] —
  **로컬 D-023은 원격 재기준을 모르는 승인이었음, 재확정 전 실사용 보류 권고**)
  ⑵ 리뷰 게이트 정본(캡슐 vs run-scoped) ⑶ 결정 로그 위치·ADR 존폐·D-020~023
  재번호 ⑷ vsp 편입(D-018 재론 — 원격 브리핑 완비) → 통합 세션 1~2회(권고:
  원격 베이스 + 로컬 우월 자산[엔진 4.13.15·vsp lock 94·FI 팩·지식 이식] 이식).
  스프린트 잔여 결정 ②③④⑤⑦은 통합 뒤로. 전 기간: 5-12 금지(원격 S1~S4가
  사실상 대체 후보 — ⑴ 종속) · 동결 드리프트 1파일 사용자 판단.
