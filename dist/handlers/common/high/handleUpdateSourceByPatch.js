...
exports.handleUpdateSourceByPatch = handleUpdateSourceByPatch;
const clients_1 = require("../../../lib/clients");
const sourcePatch_1 = require("../../../lib/sourcePatch");
...
const handleUpdateInclude_1 = require("../../include/high/handleUpdateInclude");
    ...
    inputSchema: {
        ...
        properties: {
            object_type: {
                ...
                description: 'ABAP object kind to patch: CLAS (class), PROG (program), INTF (interface), INCL (include), FUNC (function module).',
            },
            object_name: {
                type: 'string',
            ...
            function_group: {
                type: 'string',
                description: "Function group name. Required when object_type is 'FUNC'.",
            },
            old_string: {
                type: 'string',
                description: 'Exact text to find in the current source (whitespace-sensitive). Must match exactly once unless replace_all is true.',
            },
            new_string: {
                type: 'string',
            ...
            transport_request: {
                type: 'string',
                description: 'Transport request number, passed through to the delegated update handler.',
            },
            activate: {
                type: 'boolean',
                description: 'Activate the object after the patched source is written. Default: false.',
            },
        },
        required: ['object_type', 'object_name', 'old_string', 'new_string'],
    },
};
function extractReadSource(readResult, objectName, label) {
...
async function fetchCurrentSource(context, objectType, objectName, functionGroup) {
    ...
    if (objectType === 'INCL') {
        ...
        if (includeResult.isError) {
            throw new Error(`Failed to fetch current source for include ${objectName}: ${includeResult.content?.[0]?.text ?? 'unknown error'}`);
        }
    ...
    const readResult = await client
        .getFunctionModule()
        .read({ functionModuleName: objectName, functionGroupName: functionGroup }, 'active');
    return extractReadSource(readResult, objectName, 'Function module');
}
async function delegateUpdate(context, objectType, objectName, functionGroup, newSource, transportRequest, activate) {
    switch (objectType) {
        case 'CLAS':
            return (0, handleUpdateClass_1.handleUpdateClass)(context, {
        ...
        case 'PROG':
            return (0, handleUpdateProgram_1.handleUpdateProgram)(context, {
        ...
        case 'INTF':
            return (0, handleUpdateInterface_1.handleUpdateInterface)(context, {
        ...
        case 'INCL':
            return (0, handleUpdateInclude_1.handleUpdateInclude)(context, {
                include_name: objectName,
                source_code: newSource,
                transport_request: transportRequest,
                activate,
        ...
        case 'FUNC':
            return (0, handleUpdateFunctionModule_1.handleUpdateFunctionModule)(context, {
...
async function handleUpdateSourceByPatch(context, args) {
    const { logger } = context;
    try {
        const { object_type, object_name, function_group, old_string, new_string, replace_all, transport_request, activate, } = args ?? {};
        if (!object_type ||
            !object_name ||
            old_string === undefined ||
            new_string === undefined) {
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, 'object_type, object_name, old_string, and new_string are required');
        }
        const validTypes = [
            'CLAS',
            'PROG',
        ...
        if (!validTypes.includes(object_type)) {
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, `Unsupported object_type '${object_type}'. Must be one of: ${validTypes.join(', ')}`);
        }
        ...
        logger?.info(`UpdateSourceByPatch: object_type=${object_type}, object_name=${objectName}, replace_all=${shouldReplaceAll}, activate=${shouldActivate}`);
        // Step 1: fetch current source
        const currentSource = await fetchCurrentSource(context, object_type, objectName, functionGroup);
        // Step 2: locate + apply patch (pure logic; throws on not-found / not-unique)
        let patch;
        try {
            patch = (0, sourcePatch_1.applySourcePatch)(currentSource, old_string, new_string, shouldReplaceAll);
        }
        catch (patchError) {
            return (0, utils_1.return_error)(new Error(`${patchError.message} (object: ${object_type} ${objectName})`));
        }
        // Step 3: compact diff preview around the first replaced region
        const diffPreview = (0, sourcePatch_1.buildDiffPreview)(currentSource, patch.newSource, patch.firstMatchIndex, old_string, new_string);
        ...
        const delegateResult = await delegateUpdate(context, object_type, objectName, functionGroup, patch.newSource, transport_request, shouldActivate);
        ...
        try {
            ...
            const delegatePayload = typeof delegateText === 'string' ? JSON.parse(delegateText) : undefined;
            if (delegatePayload) {
...
//# sourceMappingURL=handleUpdateSourceByPatch.js.map