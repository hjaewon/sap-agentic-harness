...
class ConfigLoader {
    ...
    static load() {
        ...
        const cliArgs = ArgumentsParser_js_1.ArgumentsParser.parse();
        // Load YAML config if --config is specified
        let yamlConfig = null;
        if (cliArgs.config) {
            try {
                // Generate template if file doesn't exist
                const templateGenerated = (0, yamlConfig_js_1.generateConfigTemplateIfNeeded)(cliArgs.config);
                if (templateGenerated) {
                    // Template was generated, exit (handled by caller)
                    throw new Error('Config template generated');
                }
                // Load YAML config
                yamlConfig = (0, yamlConfig_js_1.loadYamlConfig)(cliArgs.config);
            }
            catch (error) {
        ...
        const config = {
            ...
            transportConfig,
            transport: transportConfig.type === 'streamable-http'
                ? 'http'
     ...
     */
    static mergeConfig(cliArgs, yamlConfig) {
        if (!yamlConfig) {
            return cliArgs;
        }
        ...
        if (yamlConfig.http) {
            ...
            if (!merged.httpEnableDnsProtection &&
        ...
        if (yamlConfig.sse) {
            ...
            if (!merged.sseEnableDnsProtection &&
     ...
     */
    static buildTransportConfig(args) {
